...
**2. Qwen2.5-VL-3B-Instruct 模型：Beam 位置越靠后，分数越低（绝对值越大）**

- **趋势**：从 Beam 1 的 -9.29e-08 下降到 Beam 5 的 -1.18e-07，绝对值增加了约 27.0%
- **原因分析**：
  - Qwen2.5-VL-3B-Instruct 的分数都是接近0的负数，这可能与分数计算方式有关
  - 虽然绝对值很小，但趋势与 Flamingo-3B 一致，都表现出 Beam 位置越靠后分数越低的规律
...
- **模型差异**：
  ...
  - **Qwen2.5-VL-3B-Instruct**：分数范围很小（约 -1.2e-07 到 -9.3e-08），不同 Beam 位置的质量差异相对较小
- **实际应用建议**：
