#!/bin/bash
# 从 RCE checkpoint 继续训练 GRPO（KL_BETA=0.1，解冻所有层，学习率 1e-5）
# 实验配置：更大的学习率 + 更小的 KL_BETA
...
# 输出目录（KL_BETA=0.1，学习率 1e-5，解冻版本）
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_RandSampler_Qwen2_5-VL-3B-Instruct_kl010_unfrozen_lr1e5"

# 训练参数（更大的学习率 + 更小的 KL_BETA）
RCE_EPOCHS=0          # 不做 RCE，直接加载已训练好的 RCE checkpoint
GRPO_EPOCHS=${grpo_epochs}
GRPO_LR=1e-5         # 更大的学习率（原来是 5e-6）
KL_BETA=0.1          # 更小的 KL_BETA（原来是 0.15）
BATCH_SIZE=1

echo "=========================================="
echo "从 RCE epoch ${rce_epoch} 开始，使用 KL_BETA=0.1，学习率=1e-5 训练 GRPO（解冻所有层）"
echo "=========================================="
echo "  - RCE checkpoint: ${RCE_CKPT}"
echo "  - RCE 目录: ${rce_dir}"
echo "  - RCE epochs: ${RCE_EPOCHS} (直接加载)"
echo "  - GRPO epochs: ${GRPO_EPOCHS}"
echo "  - GRPO LR: ${GRPO_LR} (更大的学习率)"
echo "  - KL beta: ${KL_BETA} (更小的 KL_BETA，允许更大的策略更新)"
echo "  - GPU: ${gpu_id}"
echo "  - 冻结 backbone: ❌ 否（训练所有参数）"
echo "=========================================="
...
# 注意：不传递 --freeze_backbone_in_grpo 参数，这样所有参数都会被训练
...
echo "=========================================="
echo "下一步：运行评估脚本"
echo "  bash scripts/eval_grpo_kl010_unfrozen_lr1e5.sh 1 ${gpu_id} 200"
echo "=========================================="
