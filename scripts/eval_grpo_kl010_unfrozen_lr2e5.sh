#!/bin/bash
# 评估 KL_BETA=0.1，学习率=2e-5，解冻版本的 GRPO 模型
...
# KL_BETA=0.1，学习率=2e-5，解冻版本的 checkpoint 路径
CKPT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_RandSampler_Qwen2_5-VL-3B-Instruct_kl010_unfrozen_lr2e5"
...
if [ ! -f "$PT_PATH" ]; then
    ...
    echo "请先完成 GRPO 训练（KL_BETA=0.1，学习率=2e-5，解冻版本）"
    exit 1
fi
...
echo "=========================================="
echo "GRPO Epoch ${GRPO_EPOCH} 评估 (KL_BETA=0.1，学习率=2e-5，解冻版本)"
...
echo ""
echo "✓ GPU ${GPU_ID} 完成: ${TEST_NUM} 样本评估 (GRPO Epoch ${GRPO_EPOCH}, KL_BETA=0.1，学习率=2e-5，解冻版本)"
