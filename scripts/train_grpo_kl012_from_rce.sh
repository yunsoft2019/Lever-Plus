#!/bin/bash
# 从 RCE checkpoint 继续训练 GRPO（KL_BETA=0.12）
# 与 train_v3_kl012.sh 配合使用，用于在 RCE 训练后继续 GRPO 训练
...
# 输出目录（与 train_v3_kl012.sh 一致）
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_RandSampler_Qwen2_5-VL-3B-Instruct_kl012"

# 训练参数（与参考配置一致，KL_BETA=0.12）
...
echo "=========================================="
echo "从 RCE epoch ${rce_epoch} 开始，使用 KL_BETA=0.12 训练 GRPO"
echo "=========================================="
...
if [ ! -f "$RCE_CKPT" ]; then
    ...
    ls -1 "${OUTPUT_DIR}"/rce_epoch*.pt 2>/dev/null || echo "  无"
    exit 1
fi
...
echo "=========================================="
echo "下一步：运行评估脚本"
echo "  bash scripts/eval_grpo_kl012.sh 1 ${gpu_id} 100"
echo "=========================================="
