#!/bin/bash
# RL数据多样性补全脚本 - 专门用于补全All-One和All-0.6查询（使用增强策略）
...
GPU_ID=2  # 改用GPU2（空闲），如果GPU2被占用，可以改为3或4
...
# 从完整数据中提取All-One和All-0.6的query
INPUT_RL_DATA="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_merged_fixed_candidate_pool.json"
...
OUTPUT_PATH="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all1_all06_diverse.json"
REPORT_PATH="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all1_all06_diverse_report.json"
...
echo "=========================================="
echo "RL数据多样性补全 - All-One和All-0.6查询专用（GPU${GPU_ID}）"
echo "=========================================="
echo "输入数据: ${INPUT_RL_DATA}"
echo "输出数据: ${OUTPUT_PATH} (将自动添加时间戳)"
echo "处理query数量: All-One和All-0.6查询"
echo ""
echo "【增强策略】"
echo "  - max_eval_budget_all1: 40 (从8增加到40)"
echo "  - max_eval_budget_all06: 40 (从8增加到40)"
echo "  - E_per_round: 12 (从4增加到12)"
echo "  - B值范围: [200, 400, 600, 800, 1000] (多轮次策略)"
echo "  - L值范围: [50, 100, 200, 400, 600] (All-0.6补1时使用)"
echo "  - 添加Level-2策略（SFT模型生成候选）"
echo ""
