#!/bin/bash
# V4-5 方案：GRU Pointer Decoder + Additive/Bilinear Attention 打分头
#
# 核心改动：
# 1. 把 dot-product 换成 Additive (Bahdanau) 或 Bilinear Attention 打分头
# 2. 解决 query embedding 和 candidate embedding 不完全同空间的问题
# 3. 提升打分的可表达性
#
# 与方案五相同的配置（RCE epochs 增加到 15，因为 Additive Attention 参数需要更多训练）：
# - 使用相同的 RL 数据：rl_data_k64_v3_balanced.json
# - KL_BETA=0.1
# - GRPO_EPOCHS=50
# - GRPO_LR=5e-6
# - RCE_EPOCHS=15（增加，因为 Additive/Bilinear Attention 参数是随机初始化的）
#
# 使用方法:
#   bash scripts/train_v3_plan_v4_5.sh [gpu_id] [attention_type]
#
# 示例:
...
# 解析参数
gpu_id=${1:-0}
attention_type=${2:-additive}  # 默认使用 additive attention
...
echo "=========================================="
echo "V4-5 方案：GRU + ${attention_type^^} Attention 打分头"
echo "=========================================="
echo "核心改动："
echo "  - GRUCell decoder: history-aware 能力"
echo "  - Step embedding: 不同 step 学到不同策略"
echo "  - ${attention_type^^} Attention: 替代 dot-product 打分"
if [ "$attention_type" = "additive" ]; then
    echo "    score = v^T * tanh(W_q * q + W_c * c)"
elif [ "$attention_type" = "bilinear" ]; then
    echo "    score = q^T * W * c"
fi
echo "  - 解决 embedding 不完全同空间的问题"
echo ""
...
echo "  - GRPO_EPOCHS=50"
echo "  - GRPO_LR=5e-6"
echo "  - RCE_EPOCHS=15（增加，因为 Additive/Bilinear Attention 参数需要更多训练）"
echo "=========================================="
...
# 输出目录（根据 attention_type 区分）
output_dir="${PROJECT_DIR}/results/okvqa/model_cpk/v3_plan_v4_5_${attention_type}"
mkdir -p "$output_dir"
...
# 执行训练
CUDA_VISIBLE_DEVICES=${gpu_id} python -m lever_lm.workflows.grpo_post_train_v4_5 \
...
echo ""
echo "=========================================="
echo "✓ V4-5 (${attention_type^^}) 训练完成！"
echo "=========================================="
echo "Checkpoint 保存在: $output_dir"
echo ""
echo "下一步：找到最优 epoch 并运行推理评估"
echo "  1. 查看训练日志，找到 Val Loss 最小的 epoch"
echo "  2. 运行推理: bash scripts/inference_v4_5.sh [gpu_id] [best_epoch] [attention_type]"
echo "=========================================="
