...
# KL_BETA=0.145 的 checkpoint 路径
CKPT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_RandSampler_Qwen2_5-VL-3B-Instruct_kl0145"
...
if [ ! -f "$PT_PATH" ]; then
    ...
    echo "请先完成 GRPO 训练（KL_BETA=0.145）"
    exit 1
fi
...
echo "=========================================="
echo "GRPO Epoch ${GRPO_EPOCH} 评估 (KL_BETA=0.145)"
...
echo ""
echo "✓ GPU ${GPU_ID} 完成: ${TEST_NUM} 样本评估 (GRPO Epoch ${GRPO_EPOCH}, KL_BETA=0.145)"
