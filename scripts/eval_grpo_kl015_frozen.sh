#!/bin/bash
# 评估 KL_BETA=0.15 冻结版本的 GRPO 模型（真正的 0.15）
...
# KL_BETA=0.15 冻结版本的 checkpoint 路径
CKPT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_RandSampler_Qwen2_5-VL-3B-Instruct_kl015_frozen"
...
if [ ! -f "$PT_PATH" ]; then
    ...
    echo "请先完成 GRPO 训练（KL_BETA=0.15，冻结版本）"
    exit 1
fi
...
echo "=========================================="
echo "GRPO Epoch ${GRPO_EPOCH} 评估 (KL_BETA=0.15，冻结版本，真正的 0.15)"
...
echo ""
echo "✓ GPU ${GPU_ID} 完成: ${TEST_NUM} 样本评估 (GRPO Epoch ${GRPO_EPOCH}, KL_BETA=0.15，冻结版本)"
