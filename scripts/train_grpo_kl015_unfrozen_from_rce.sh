#!/bin/bash
# 从 RCE checkpoint 继续训练 GRPO（KL_BETA=0.15，解冻所有层）
# 与 train_grpo_kl015_from_rce.sh 的区别：不冻结 backbone，训练所有参数
...
# 输出目录（KL_BETA=0.15 解冻版本的输出目录）
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_RandSampler_Qwen2_5-VL-3B-Instruct_kl015_unfrozen"

# 训练参数（与参考配置一致，KL_BETA=0.15，但不冻结 backbone）
...
echo "=========================================="
echo "从 RCE epoch ${rce_epoch} 开始，使用 KL_BETA=0.15 训练 GRPO（解冻所有层）"
echo "=========================================="
...
echo "  - GPU: ${gpu_id}"
echo "  - 冻结 backbone: ❌ 否（训练所有参数：input_proj, query_proj, cand_proj + cross_attn + attn_norms）"
echo "=========================================="
...
echo "=========================================="
echo "下一步：运行评估脚本"
echo "  bash scripts/eval_grpo_kl015_unfrozen.sh 1 ${gpu_id} 200"
echo "=========================================="
