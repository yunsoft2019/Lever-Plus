#!/bin/bash
# RL数据多样性补全脚本 - 专门用于补全All-Zero查询（使用增强策略）
...
INPUT_RL_DATA="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all0_only.json"
...
OUTPUT_PATH="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all0_only_diverse.json"
REPORT_PATH="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all0_only_diverse_report.json"
...
echo "=========================================="
echo "RL数据多样性补全 - All-Zero查询专用（GPU${GPU_ID}）"
echo "=========================================="
echo "输入数据: ${INPUT_RL_DATA}"
echo "输出数据: ${OUTPUT_PATH} (将自动添加时间戳)"
echo "处理query数量: 全部All-Zero查询 (约196个)"
echo ""
echo "【增强策略】"
echo "  - max_eval_budget_all0: 50 (从18增加到50)"
echo "  - L值范围: [100, 200, 300, 400, 500] (从[80,150,200]扩大)"
echo "  - 每轮评测数: 10 (从6增加到10)"
echo ""
