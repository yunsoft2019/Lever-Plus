#!/bin/bash
# 使用Per-Query候选池的RL数据重新训练checkpoint
# 使用方法: bash scripts/train_v3_with_per_query_candidate_pool.sh [GPU_ID]
...
# 文件路径（使用修复后的数据）
RL_DATA="${PROJECT_ROOT}/results/${DATASET_NAME}/generated_data/rl_data_RandSampler_v4_strictEval_merged_fixed_candidate_pool.json"
...
# 输出目录（使用per_query_candidate_pool后缀）
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_per_query_candidate_pool"
...
echo "=========================================="
echo "使用Per-Query候选池的RL数据重新训练checkpoint"
echo "=========================================="
echo "GPU ID: ${GPU_ID}"
echo "RL数据: ${RL_DATA}"
...
echo ""
echo "【关键修复】"
echo "  - 每个query使用自己独立的候选池（从该query的pointer_candidates中提取）"
echo "  - 限制每个query的候选池大小为64（与RL数据生成时一致）"
echo "  - 训练和推理时都使用per-query候选池，确保action space一致"
echo ""
echo "训练参数："
...
echo ""
echo "可以使用以下命令进行推理："
echo "bash scripts/grpo_inference_200.sh ${GPU_ID}"
