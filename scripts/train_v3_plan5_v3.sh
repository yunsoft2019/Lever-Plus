#!/bin/bash
# 方案五 v3：使用增强数据 + 禁用自适应 KL + 固定 KL_BETA=0.5
# 
# 改进点（相比 v2）：
# 1. 禁用自适应 KL 调整（--disable_adaptive_kl）
# 2. 使用固定的 KL_BETA=0.5
#
# v2 问题：自适应 KL 把 beta 调到了 10.0，导致 KL 仍然很高
# v3 目标：使用固定 beta，严格控制 KL 在 0.1-0.5 范围内
#
# 使用方法:
#   bash scripts/train_v3_plan5_v3.sh [gpu_id]
...
echo "=========================================="
echo "方案五 v3：增强数据 + 禁用自适应 KL"
echo "=========================================="
echo "核心配置："
echo "  - KL_BETA=0.5 (固定值)"
echo "  - --disable_adaptive_kl (禁用自适应调整)"
echo "  - GRPO_EPOCHS=3"
echo "  - GRPO_LR=5e-6"
echo "=========================================="
...
output_dir="./results/okvqa/model_cpk/v3_plan5_v3_fixed_kl05"
mkdir -p "$output_dir"
