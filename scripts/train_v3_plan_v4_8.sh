...
#
# 核心改动：
# 1. 同时维护 S 个"slot"，slots 之间 self-attn 协同分工
# 2. 每个 slot 生成一行 logits，实现并行预测
# 3. 推理时使用 greedy mask 保证不重复
#
# 训练配置：
...
#
# 使用方法:
#   bash scripts/train_v3_plan_v4_8.sh [gpu_id] [num_slot_layers]
#
# 示例:
#   bash scripts/train_v3_plan_v4_8.sh 4 2
#   bash scripts/train_v3_plan_v4_8.sh 4 3
...
# 解析参数
gpu_id=${1:-0}
num_slot_layers=${2:-2}  # 默认使用 2 层 slot self-attention
...
echo "=========================================="
echo "V4-8 方案：Slot/Set Decoder (${num_slot_layers} layers)"
echo "=========================================="
echo "核心改动："
echo "  - 同时维护 S 个 slot，并行预测"
echo "  - Slot Self-Attention: ${num_slot_layers} 层"
echo "  - Slot-Candidate Cross-Attention: 启用"
echo "  - 推理时使用 greedy mask 保证不重复"
echo ""
echo "训练配置："
...
# 输出目录
output_dir="${PROJECT_DIR}/results/okvqa/model_cpk/v3_plan_v4_8_slots${num_slot_layers}"
mkdir -p "$output_dir"
...
echo "  - GPU: ${gpu_id}"
echo "  - Num Slot Layers: ${num_slot_layers}"
echo ""

# 构建训练命令
train_cmd="CUDA_VISIBLE_DEVICES=${gpu_id} python -m lever_lm.workflows.grpo_post_train_v4_8 \
...
echo ""
echo "=========================================="
echo "✓ V4-8 (Slot Decoder ${num_slot_layers} layers) 训练完成！"
echo "=========================================="
echo "Checkpoint 保存在: $output_dir"
echo ""
echo "下一步：找到最优 epoch 并运行推理评估"
echo "  1. 查看训练日志，找到 Val Loss 最小的 epoch"
echo "  2. 运行推理: bash scripts/inference_v4_8.sh [gpu_id] [stage] [epoch] [num_slot_layers]"
echo "=========================================="
