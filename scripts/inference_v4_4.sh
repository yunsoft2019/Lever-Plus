...
# 使用方法: bash scripts/inference_v4_4.sh [gpu_id] [epoch]
#
# 参数:
#   gpu_id: GPU 编号（默认 0）
...
#
# 示例:
#   bash scripts/inference_v4_4.sh 4 1
...
# V4-4 checkpoint 目录
CKPT_DIR="${PROJECT_DIR}/results/okvqa/model_cpk/v3_plan_v4_4"
...
if [ ! -d "$CKPT_DIR" ]; then
    ...
    echo "请先运行训练: bash scripts/train_v3_plan_v4_4.sh"
    exit 1
fi

# 构建 checkpoint 路径
V4_4_CKPT="${CKPT_DIR}/grpo_epoch${epoch}.pt"
...
# 检查 checkpoint 是否存在
if [ ! -f "$V4_4_CKPT" ]; then
    echo "错误: Checkpoint 文件不存在: $V4_4_CKPT"
    echo ""
    echo "可用的 checkpoint:"
...
if [ ! -f "$V2_CKPT" ]; then
    ...
    python ${PROJECT_DIR}/scripts/convert_v4_4_to_v2_format.py \
        --v4_4_ckpt "$V4_4_CKPT" \
        --output "$V2_CKPT"
    if [ ! -f "$V2_CKPT" ]; then
        echo "转换失败，请检查错误信息"
...
# 设置模型类型为 V4-4
export LEVER_LM_MODEL_TYPE="v4_4"

echo ""
echo "Checkpoint: $LEVER_LM_CHECKPOINT_PATH"
echo "Model Type: V4-4 (Candidate Set Encoder + GRU + MMR)"
echo ""
