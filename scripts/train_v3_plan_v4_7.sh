#!/bin/bash
# V4-7 方案：GRU Pointer Decoder + (N)DPP / log-det 风格集合增益
#
# 核心改动：
# 1. 使用低秩特征做 logdet 近似增益
# 2. 用 "与已选集合的最大相似度" 近似 logdet 增益
# 3. 强集合建模，增强多样性选择能力
#
...
# - GRPO_EPOCHS=50
# - GRPO_LR=5e-6
# - RCE_EPOCHS=15（增加，因为 DPP 参数需要更多训练）
#
# 使用方法:
#   bash scripts/train_v3_plan_v4_7.sh [gpu_id] [dpp_rank]
#
# 示例:
#   bash scripts/train_v3_plan_v4_7.sh 4 32
#   bash scripts/train_v3_plan_v4_7.sh 4 64
...
echo "=========================================="
echo "V4-7 方案：GRU + DPP (rank=${dpp_rank})"
echo "=========================================="
echo "核心改动："
echo "  - GRUCell decoder: history-aware 能力"
echo "  - Step embedding: 不同 step 学到不同策略"
echo "  - DPP 机制: ${dpp_rank} 维低秩投影"
echo "  - 使用 log-det 近似增益"
echo "  - 强化集合多样性建模"
echo ""
...
echo "  - GRPO_EPOCHS=50"
echo "  - GRPO_LR=5e-6"
echo "  - RCE_EPOCHS=15（增加，因为 DPP 参数需要更多训练）"
echo "=========================================="
...
# 输出目录（根据 dpp_rank 区分）
output_dir="${PROJECT_DIR}/results/okvqa/model_cpk/v3_plan_v4_7_rank${dpp_rank}"
mkdir -p "$output_dir"
...
# 执行训练
CUDA_VISIBLE_DEVICES=${gpu_id} python -m lever_lm.workflows.grpo_post_train_v4_7 \
...
echo ""
echo "=========================================="
echo "✓ V4-7 (DPP rank=${dpp_rank}) 训练完成！"
echo "=========================================="
echo "Checkpoint 保存在: $output_dir"
echo ""
echo "下一步：找到最优 epoch 并运行推理评估"
echo "  1. 查看训练日志，找到 Val Loss 最小的 epoch"
echo "  2. 运行推理: bash scripts/inference_v4_7.sh [gpu_id] [best_epoch] [dpp_rank]"
echo "=========================================="
