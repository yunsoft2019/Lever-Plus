#!/bin/bash
# 方案 7.2 训练脚本：PPO Clip + 中等 KL 惩罚
#
# 核心改动（相比方案 7.1）：
# 1. 保持 use_ppo_clip_only=true
# 2. 增大 KL 惩罚 kl_beta=0.05（从 0.01 提升到 0.05）
#
# 预期效果：
# - 保留 PPO Clip 的梯度增强效果
# - 通过更强的 KL 惩罚控制 KL 爆炸
# - 期望 Epoch 1 KL < 10（而不是 17.8）
...
export KL_BETA=0.05                  # 方案 7.2 核心：中等 KL 惩罚（从 0.01 提升到 0.05）
...
echo "=========================================="
echo "方案 7.2 训练：PPO Clip + 中等 KL 惩罚"
echo "=========================================="
echo "配置："
...
echo "  - KL_BETA=0.05（中等 KL 惩罚，控制爆炸）"
echo "  - GRPO_EPOCHS=50"
echo "  - GPU: ${gpu_id}"
echo "=========================================="
...
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_plan7_2_ppo_clip_medium_kl"
