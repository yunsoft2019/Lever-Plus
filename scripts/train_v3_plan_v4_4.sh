#!/bin/bash
# V4-4 方案：Candidate Set Encoder（Self-Attn）+ GRU Pointer Decoder + MMR
#
# 核心改动：
# 1. 在 V4-3 基础上添加 Candidate Self-Attention
# 2. 让候选之间先"互相看一眼"，学习重复/冗余关系
# 3. 结合 GRU decoder 和 MMR 多样性残差
#
# 与方案五相同的配置：
...
#
# 使用方法:
#   bash scripts/train_v3_plan_v4_4.sh [gpu_id]
#
# 示例:
#   bash scripts/train_v3_plan_v4_4.sh 4
...
echo "=========================================="
echo "V4-4 方案：Candidate Set Encoder + GRU + MMR"
echo "=========================================="
echo "核心改动："
echo "  - Candidate Self-Attention: 候选之间先交互"
...
# 输出目录
output_dir="${PROJECT_DIR}/results/okvqa/model_cpk/v3_plan_v4_4"
mkdir -p "$output_dir"
...
# 执行训练
CUDA_VISIBLE_DEVICES=${gpu_id} python -m lever_lm.workflows.grpo_post_train_v4_4 \
...
echo "  2. 运行推理: bash scripts/inference_v4_4.sh [gpu_id] [best_epoch]"
echo "=========================================="
