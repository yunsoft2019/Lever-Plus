#!/bin/bash
# 测试RCE模型效果
# 使用方法: bash scripts/test_rce_model.sh [GPU_ID] [TEST_NUM] [SHOT_NUM]
#
# 示例:
#   bash scripts/test_rce_model.sh 4 200 2

set -e

# 参数
GPU_ID=${1:-4}
TEST_NUM=${2:-200}
SHOT_NUM=${3:-2}

# 路径配置
PROJECT_ROOT="/mnt/share/yiyun/Projects/Lever-Plus"
DATASET="okvqa"

# RCE模型checkpoint（使用epoch5，RCE训练的最后一步）
RCE_CKPT="${PROJECT_ROOT}/results/${DATASET}/model_cpk/v3_large_scale/rce_epoch5.pt"
QUERY_EMB="${PROJECT_ROOT}/results/${DATASET}/cache/query_embeddings.pt"
BEAM_DATA="${PROJECT_ROOT}/results/${DATASET}/generated_data/rl_data_v4_large_scale.json"
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET}/eval/v3_large_scale_rce"

# VQA模型配置
VQA_MODEL="Qwen2.5-VL-3B-Instruct"
DATASET_NAME="okvqa"

echo "=========================================="
echo "测试RCE模型效果"
echo "=========================================="
echo "GPU: ${GPU_ID}"
echo "RCE Checkpoint: ${RCE_CKPT}"
echo "Query Embeddings: ${QUERY_EMB}"
echo "Beam Data: ${BEAM_DATA}"
echo "Test Num: ${TEST_NUM}"
echo "Shot Num: ${SHOT_NUM}"
echo "Output Dir: ${OUTPUT_DIR}"
echo "=========================================="

# 检查checkpoint是否存在
if [ ! -f "$RCE_CKPT" ]; then
    echo "错误: RCE checkpoint不存在: $RCE_CKPT"
    echo ""
    echo "可用的checkpoint:"
    ls -lh "${PROJECT_ROOT}/results/${DATASET}/model_cpk/v3_large_scale/"*.pt 2>/dev/null | head -10
    exit 1
fi

# 检查其他文件
if [ ! -f "$QUERY_EMB" ]; then
    echo "错误: Query embeddings不存在: $QUERY_EMB"
    exit 1
fi

if [ ! -f "$BEAM_DATA" ]; then
    echo "错误: Beam data不存在: $BEAM_DATA"
    exit 1
fi

echo ""
echo "开始测试..."
echo ""

cd "${PROJECT_ROOT}"

# 创建输出目录
mkdir -p "${OUTPUT_DIR}"

# 运行评估
CUDA_VISIBLE_DEVICES=${GPU_ID} python -m lever_lm.workflows.evaluate_v3 \
    --grpo_ckpt "${RCE_CKPT}" \
    --img_emb "${QUERY_EMB}" \
    --beam_data "${BEAM_DATA}" \
    --dataset "${DATASET_NAME}" \
    --model_name "${VQA_MODEL}" \
    --shot_num ${SHOT_NUM} \
    --test_num ${TEST_NUM} \
    --output_dir "${OUTPUT_DIR}" \
    --batch_size 32 \
    --device cuda:0

echo ""
echo "=========================================="
echo "✓ RCE模型测试完成！"
echo "结果保存在: ${OUTPUT_DIR}"
echo "=========================================="
echo ""
echo "查看结果:"
echo "  cat ${OUTPUT_DIR}/results.json"
echo "  cat ${OUTPUT_DIR}/vqa_accuracy.txt"

