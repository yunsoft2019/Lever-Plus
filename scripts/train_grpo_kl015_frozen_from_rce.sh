#!/bin/bash
# 从 RCE checkpoint 继续训练 GRPO（KL_BETA=0.15，冻结 backbone）
# 这是真正的 KL_BETA=0.15 冻结版本，用于公平对比
...
rce_dir=${4:-kl012}   # RCE checkpoint 所在目录（默认 kl012）
...
# 从指定的 RCE checkpoint 开始（使用 kl012 的 RCE checkpoint，确保公平对比）
...
# 输出目录（KL_BETA=0.15 冻结版本的输出目录）
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_RandSampler_Qwen2_5-VL-3B-Instruct_kl015_frozen"

# 训练参数（KL_BETA=0.15，冻结 backbone）
RCE_EPOCHS=0          # 不做 RCE，直接加载已训练好的 RCE checkpoint
GRPO_EPOCHS=${grpo_epochs}
GRPO_LR=5e-6          # 与参考配置一致
KL_BETA=0.15          # 真正的 KL_BETA=0.15
BATCH_SIZE=1

echo "=========================================="
echo "从 RCE epoch ${rce_epoch} 开始，使用 KL_BETA=0.15 训练 GRPO（冻结 backbone）"
echo "=========================================="
...
echo "  - KL beta: ${KL_BETA} ✅ 真正的 0.15"
echo "  - GPU: ${gpu_id}"
echo "  - 冻结 backbone: ✅ 是（只训练 50.1% 参数：cross_attn + attn_norms）"
echo "  - 数据文件: rl_data_k64_v3.json（统一数据）"
echo "=========================================="
...
# 注意：传递 --freeze_backbone_in_grpo 参数，冻结投影层
...
echo "=========================================="
echo "下一步：运行评估脚本"
echo "  bash scripts/eval_grpo_kl015_frozen.sh 1 ${gpu_id} 200"
echo "=========================================="
