#!/bin/bash
# V3 训练脚本（KL_BETA=0.18版本）
# 使用方法: bash scripts/train_v3_kl018.sh <task> <dataset> <gpu_id> <lever_lm> <sampler> <beam_model>
# 示例: bash scripts/train_v3_kl018.sh vqa okvqa_local 0 query_img_text_icd_img_text rand_sampler qwen2.5_vl_3B
#
# 该脚本与 train_v3.sh 完全一致，只将 KL_BETA 从 0.15 改为 0.18
# 其他所有配置保持不变
...
output_dir="./results/${dataset_name}/model_cpk/v3_${sampler_name}_${model_name}${reward_suffix}_kl018"
...
kl_beta=${KL_BETA:-0.18}  # 从 0.15 改为 0.18
...
echo "  KL Beta: ${kl_beta} (从 0.15 改为 0.18)"
