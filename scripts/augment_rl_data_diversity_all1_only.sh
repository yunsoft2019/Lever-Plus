#!/bin/bash
# RL数据多样性补全脚本 - 专门用于补全All-One查询（使用增强策略）
...
INPUT_RL_DATA="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all1_only.json"
...
OUTPUT_PATH="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all1_only_diverse.json"
REPORT_PATH="${PROJECT_ROOT}/results/okvqa/generated_data/rl_data_RandSampler_v4_strictEval_all1_only_diverse_report.json"
...
echo "=========================================="
echo "RL数据多样性补全 - All-One查询专用（GPU${GPU_ID}）"
echo "=========================================="
echo "输入数据: ${INPUT_RL_DATA}"
echo "输出数据: ${OUTPUT_PATH} (将自动添加时间戳)"
echo "处理query数量: 全部All-One查询 (约177个)"
echo ""
echo "【补全策略】"
echo "  - All-One查询需要补出0或0.6的候选"
echo "  - 使用bottom pair和mix pair策略"
echo ""
