#!/bin/bash
# 使用合并后的RL数据重新训练checkpoint（优化版）
# 使用方法: bash scripts/train_v3_with_merged_rl_data_optimized.sh [GPU_ID]
...
# 输出目录（使用optimized后缀）
OUTPUT_DIR="${PROJECT_ROOT}/results/${DATASET_NAME}/model_cpk/v3_merged_optimized"
...
echo "=========================================="
echo "使用合并后的RL数据重新训练checkpoint（优化版）"
echo "=========================================="
echo "GPU ID: ${GPU_ID}"
