#!/bin/bash
# V3 训练脚本（KL_BETA=0.12版本）
# 使用方法: bash scripts/train_v3_kl012.sh <task> <dataset> <gpu_id> <lever_lm> <sampler> <beam_model>
# 示例: bash scripts/train_v3_kl012.sh vqa okvqa_local 0 query_img_text_icd_img_text rand_sampler qwen2.5_vl_3B
#
# 该脚本与 train_v3.sh 完全一致，只将 KL_BETA 从 0.15 改为 0.12
# 其他所有配置保持不变
...
# RL 数据路径：使用与 KL_BETA=0.15 相同的数据文件，确保公平对比
rl_data_path="./results/${dataset_name}/generated_data/rl_data_k64_v3.json"
...
output_dir="./results/${dataset_name}/model_cpk/v3_${sampler_name}_${model_name}${reward_suffix}_kl012"
...
echo "  KL Beta: ${kl_beta} (从 0.15 改为 0.12)"
