#!/bin/bash
# V4-2 方案：GRU Pointer Decoder
#
# 核心改动：
# 1. 使用 GRUCell 作为状态更新机制（替代 V4-1 的 query_update_gate）
# 2. 可选 step embedding 让不同 step 学到不同策略
# 3. GRU 可以学习更复杂的"记忆/遗忘/组合策略"
#
# 与方案五相同的配置：
...
#
# 使用方法:
#   bash scripts/train_v3_plan_v4_2.sh [gpu_id]
#
# 示例:
#   bash scripts/train_v3_plan_v4_2.sh 4
...
echo "=========================================="
echo "V4-2 方案：GRU Pointer Decoder"
echo "=========================================="
echo "核心改动："
echo "  - GRUCell decoder: 更强的 history-aware 能力"
echo "  - Step embedding: 不同 step 学到不同策略"
echo "  - 比 V4-1 的简单 gate 更强"
echo ""
echo "训练配置（与方案五相同）："
...
# 输出目录
output_dir="${PROJECT_DIR}/results/okvqa/model_cpk/v3_plan_v4_2"
mkdir -p "$output_dir"
...
# 执行训练
CUDA_VISIBLE_DEVICES=${gpu_id} python -m lever_lm.workflows.grpo_post_train_v4_2 \
...
echo "  2. 运行推理: bash scripts/inference_v4_2.sh [gpu_id] [best_epoch]"
echo "=========================================="
