"""
GRPO Post-Training 训练脚本 - V4-3 版本（GRU Pointer Decoder + MMR 多样性残差）

基于 grpo_post_train_v4_2.py，使用 V4-3 模型（GRU + MMR）
...
class GRPOTrainerV4_3:
    """GRPO Post-Training 训练器 - V4-3 版本"""
    ...
    def compute_old_log_probs(self, sft_model: PointerSelectorV4_3_RL) -> Dict[int, torch.Tensor]:
        """预计算SFT模型的log概率"""
    ...
    def save_checkpoint(self, epoch: int, phase: str, metrics: Dict[str, float], verbose: bool = False):
        ...
        torch.save({
            ...
            "metrics": metrics,
            "model_type": "v4_3",
            "div_lambda_values": div_lambda_values  # V4-3 特有
        }, ckpt_path)
        if verbose:
    ...
    def train(self, sft_checkpoint: Optional[str] = None):
        """完整训练流程"""
        print("="*70)
        print("GRPO Post-Training - V4-3 (GRU + MMR 多样性残差)")
        print("="*70)
        ...
        if sft_checkpoint:
            ...
            if missing:
                print(f"[信息] V4-3 新增参数（随机初始化）: {len(missing)} 个")
                for k in list(missing)[:5]:
                    print(f"  - {k}")
                if len(missing) > 5:
                    print(f"  ... 还有 {len(missing) - 5} 个")
            
            print("✓ SFT权重加载完成（V4-3 特有参数已随机初始化）")
        
        # 创建SFT模型副本
        sft_model = PointerSelectorV4_3_RL(
        ...
        print("="*120)
        print("✓ GRPO Post-Training (V4-3) 完成！")
        print(f"  最终 div_lambda: {self.model.get_div_lambda_values()}")
        print("="*120)


def main():
    parser = argparse.ArgumentParser(description="GRPO Post-Training - V4-3")
    ...
    parser.add_argument("--output_dir", type=str, default="results/grpo_v4_3", help="输出目录")
    ...
    if pointer_candidates:
        ...
        raise ValueError("V4-3 训练需要 RL 格式数据")
    ...
    if args.img_emb:
        ...
        raise ValueError("V4-3 训练需要提供 --img_emb 参数")
    ...
    print(f"  - 架构: GRU Decoder + MMR 多样性残差 (V4-3)")
