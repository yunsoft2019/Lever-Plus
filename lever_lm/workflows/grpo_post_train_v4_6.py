"""
GRPO Post-Training 训练脚本 - V4-6 版本（GRU Pointer Decoder + Coverage / Topic 原型覆盖）

基于 grpo_post_train_v4_5.py，使用 V4-6 模型（GRU + Coverage）
...
class GRPOTrainerV4_6:
    """GRPO Post-Training 训练器 - V4-6 版本"""
    ...
    def compute_old_log_probs(self, sft_model: PointerSelectorV4_6_RL) -> Dict[int, torch.Tensor]:
        """预计算SFT模型的log概率"""
    ...
    def save_checkpoint(self, epoch: int, phase: str, metrics: Dict[str, float], verbose: bool = False):
        ...
        torch.save({
            ...
            "cover_lambda_raw": self.model.cover_lambda.item(),
            "cover_lambda_effective": torch.nn.functional.softplus(self.model.cover_lambda).item()
        }, ckpt_path)
        if verbose:
    ...
    def train(self, sft_checkpoint: Optional[str] = None):
        """完整训练流程"""
        print("="*70)
        print(f"GRPO Post-Training - V4-6 (Coverage with {self.model.num_topics} topics)")
        print("="*70)
        ...
        if sft_checkpoint:
            ...
            if missing:
                print(f"[信息] V4-6 新增参数（随机初始化）: {len(missing)} 个")
                for k in list(missing)[:5]:
                    print(f"  - {k}")
                if len(missing) > 5:
                    print(f"  ... 还有 {len(missing) - 5} 个")
            
            print("✓ SFT权重加载完成（V4-6 特有参数已随机初始化）")

        # 创建SFT模型副本
        sft_model = PointerSelectorV4_6_RL(
        ...
        print("阶段2：RCE预热")
        print("="*80)
        print(f"{'Epoch':>6} {'Train Loss':>12} {'Val Loss':>12} {'Temperature':>12} {'CoverLambda':>12}")
        print("-"*80)
        ...
        for epoch in range(self.rce_epochs):
            ...
            cover_lambda = self.model.cover_lambda.item()
            cover_lambda_effective = torch.nn.functional.softplus(self.model.cover_lambda).item()
            print(f"{epoch+1:6d} {metrics['rce_loss']:12.5f} {metrics['val_loss']:12.5f} {temperature:12.3f} {cover_lambda_effective:12.4f}")
        ...
        print("阶段3：GRPO训练")
        print("="*130)
        print(f"{'Epoch':>6} {'Train Loss':>12} {'Val Loss':>12} {'PPO Loss':>12} {'KL':>10} {'Adv Std':>10} {'Adv Max':>10} {'Beta':>8} {'CoverLambda':>12}")
        print("-"*130)
        ...
        for epoch in range(self.grpo_epochs):
            ...
            print(f"{epoch+1:6d} {metrics['grpo_loss']:12.5f} {metrics['val_loss']:12.5f} {metrics['ppo_loss']:12.5f} {metrics['kl']:10.5f} {std_adv:10.4f} {max_adv:10.4f} {self.model.kl_beta:8.4f} {cover_lambda_effective:12.4f}")
            self.save_checkpoint(epoch, "grpo", metrics)
        
        print("="*130)
        print(f"✓ GRPO Post-Training (V4-6 Coverage with {self.model.num_topics} topics) 完成！")
        print("="*130)


def main():
    parser = argparse.ArgumentParser(description="GRPO Post-Training - V4-6")
    ...
    parser.add_argument("--output_dir", type=str, default="results/grpo_v4_6", help="输出目录")
    ...
    parser.add_argument("--cover_lambda_init", type=float, default=0.0, help="覆盖增益权重初始值")
    # 其他参数
    ...
    if pointer_candidates:
        ...
        raise ValueError("V4-6 训练需要 RL 格式数据")
    ...
    if args.img_emb:
        ...
        raise ValueError("V4-6 训练需要提供 --img_emb 参数")
    ...
    print(f"  - num_topics: {args.num_topics}")
    print(f"  - cover_lambda_init: {args.cover_lambda_init}")
    print(f"  - 架构: GRU Decoder + Coverage ({args.num_topics} topics) (V4-6)")
    ...
    model = PointerSelectorV4_6_RL(
        ...
        cover_lambda_init=args.cover_lambda_init,
        label_smoothing=0.0,
        dropout=0.5
    )
