"""
GRPO Post-Training 训练脚本 - V4-2 版本（GRU Pointer Decoder）

基于 grpo_post_train.py，使用 V4-2 模型（GRU Decoder）替代 V3 模型
...
class GRPOTrainerV4_2:
    """GRPO Post-Training 训练器 - V4-2 版本"""
    ...
    def compute_old_log_probs(self, sft_model: PointerSelectorV4_2_RL) -> Dict[int, torch.Tensor]:
        """预计算SFT模型的log概率"""
    ...
    def train(self, sft_checkpoint: Optional[str] = None):
        """完整训练流程"""
        print("="*70)
        print("GRPO Post-Training - V4-2 (GRU Pointer Decoder)")
        print("="*70)
        ...
        if sft_checkpoint:
            ...
            # V4-2 需要从 V2 checkpoint 加载，需要处理参数映射
            # V2 有 query_update_gate，V4-2 有 decoder_gru 和 step_emb
            # 只加载共同的参数
            ...
            for k, v in state_dict.items():
                ...
                # 跳过 V2 特有的 query_update_gate（V4-2 使用 GRU）
            ...
            if missing:
                print(f"[信息] V4-2 新增参数（随机初始化）: {len(missing)} 个")
                for k in list(missing)[:5]:
                    print(f"  - {k}")
                if len(missing) > 5:
                    print(f"  ... 还有 {len(missing) - 5} 个")
            
            print("✓ SFT权重加载完成（V4-2 特有参数已随机初始化）")
        
        # 创建SFT模型副本用于计算old_log_probs
        sft_model = PointerSelectorV4_2_RL(
        ...
        print("="*100)
        print("✓ GRPO Post-Training (V4-2) 完成！")
        print("="*100)


def main():
    parser = argparse.ArgumentParser(description="GRPO Post-Training - V4-2")
    ...
    parser.add_argument("--output_dir", type=str, default="results/grpo_v4_2", help="输出目录")
    ...
    parser.add_argument("--no_step_emb", dest="use_step_emb", action="store_false", help="不使用step embedding")
    ...
    if pointer_candidates:
        ...
        raise ValueError("V4-2 训练需要 RL 格式数据")
    ...
    if args.img_emb:
        ...
        raise ValueError("V4-2 训练需要提供 --img_emb 参数")
