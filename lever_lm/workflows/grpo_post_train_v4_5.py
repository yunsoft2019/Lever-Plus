"""
GRPO Post-Training 训练脚本 - V4-5 版本（GRU Pointer Decoder + Additive/Bilinear Attention）

基于 grpo_post_train_v4_3.py，使用 V4-5 模型（GRU + Additive/Bilinear Attention）
...
class GRPOTrainerV4_5:
    """GRPO Post-Training 训练器 - V4-5 版本"""
    ...
    def compute_old_log_probs(self, sft_model: PointerSelectorV4_5_RL) -> Dict[int, torch.Tensor]:
        """预计算SFT模型的log概率"""
    ...
    def train(self, sft_checkpoint: Optional[str] = None):
        """完整训练流程"""
        print("="*70)
        print(f"GRPO Post-Training - V4-5 ({self.model.attention_type.upper()} Attention)")
        print("="*70)
        ...
        if sft_checkpoint:
            ...
            if missing:
                print(f"[信息] V4-5 新增参数（随机初始化）: {len(missing)} 个")
                for k in list(missing)[:5]:
                    print(f"  - {k}")
                if len(missing) > 5:
                    print(f"  ... 还有 {len(missing) - 5} 个")
            
            print("✓ SFT权重加载完成（V4-5 特有参数已随机初始化）")
        
        # 创建SFT模型副本
        sft_model = PointerSelectorV4_5_RL(
        ...
        print("="*120)
        print(f"✓ GRPO Post-Training (V4-5 {self.model.attention_type.upper()}) 完成！")
        print("="*120)


def main():
    parser = argparse.ArgumentParser(description="GRPO Post-Training - V4-5")
    ...
    parser.add_argument("--output_dir", type=str, default="results/grpo_v4_5", help="输出目录")
    ...
    if pointer_candidates:
        ...
        raise ValueError("V4-5 训练需要 RL 格式数据")
    ...
    if args.img_emb:
        ...
        raise ValueError("V4-5 训练需要提供 --img_emb 参数")
    ...
    print(f"  - 架构: GRU Decoder + {args.attention_type.upper()} Attention (V4-5)")
