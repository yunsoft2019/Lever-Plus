"""
GRPO Post-Training 训练脚本 - V4-10 版本（STOP 自适应 shot）

V4-10 方案：STOP 自适应 shot
- 添加可学习的 STOP token
- 让模型自己决定何时停止选择
- 解决"shot 越多越伤"的问题
...
class GRPOTrainerV4_10:
    """GRPO Post-Training 训练器 - V4-10 版本"""
    
    def __init__(
        self,
        model: PointerSelectorV4_10_RL,
    ...
    def compute_old_log_probs(self, sft_model: PointerSelectorV4_10_RL) -> Dict:
        """预计算SFT模型的log概率"""
    ...
    def save_checkpoint(self, epoch, phase, metrics):
        ...
        torch.save({
            ...
            "kl_beta": self.model.kl_beta, "metrics": metrics, "model_type": "v4_10",
    ...
    def train(self, sft_checkpoint: Optional[str] = None):
        """完整训练流程"""
        print("="*70)
        print(f"GRPO Post-Training - V4-10 (STOP 自适应 shot)")
        print("="*70)
        ...
        if sft_checkpoint:
            ...
            if missing:
                print(f"[信息] V4-10 新增参数（随机初始化）: {len(missing)} 个")
                for k in missing[:5]:
                    print(f"  - {k}")
            print("✓ SFT权重加载完成")

        # 创建SFT模型副本
        sft_model = PointerSelectorV4_10_RL(
...
def main():
    parser = argparse.ArgumentParser(description="GRPO Post-Training - V4-10")
    parser.add_argument("--sft_ckpt", type=str, default=None)
    parser.add_argument("--beam_data", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="results/grpo_v4_10")
