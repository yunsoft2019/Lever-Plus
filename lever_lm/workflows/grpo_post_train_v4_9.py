"""
GRPO Post-Training 训练脚本 - V4-9 版本（Two-Stage Coarse-to-Fine）
...
class GRPOTrainerV4_9:
    """GRPO Post-Training 训练器 - V4-9 版本"""
    
    def __init__(
        self,
        model: PointerSelectorV4_9_RL,
    ...
    def compute_old_log_probs(self, sft_model: PointerSelectorV4_9_RL) -> Dict:
        """预计算SFT模型的log概率"""
    ...
    def save_checkpoint(self, epoch, phase, metrics):
        ...
        torch.save({
            ...
            "kl_beta": self.model.kl_beta, "metrics": metrics, "model_type": "v4_9",
            "top_m": self.model.top_m, "refine_type": self.model.refine_type,
            "use_gru": self.model.use_gru, "use_step_emb": self.model.use_step_emb,
    ...
    def train(self, sft_checkpoint: Optional[str] = None):
        """完整训练流程"""
        print("="*70)
        print(f"GRPO Post-Training - V4-9 (Two-Stage TopM={self.model.top_m}, {self.model.refine_type})")
        print("="*70)
        ...
        if sft_checkpoint:
            ...
            if missing:
                print(f"[信息] V4-9 新增参数（随机初始化）: {len(missing)} 个")
            print("✓ SFT权重加载完成")

        # 创建SFT模型副本
        sft_model = PointerSelectorV4_9_RL(
            d_model=self.model.d_model, K=self.model.K, shot_num=self.model.shot_num,
            num_layers=self.model.num_layers, top_m=self.model.top_m, refine_type=self.model.refine_type,
            use_gru=self.model.use_gru, use_step_emb=self.model.use_step_emb,
...
def main():
    parser = argparse.ArgumentParser(description="GRPO Post-Training - V4-9")
    parser.add_argument("--sft_ckpt", type=str, default=None)
    parser.add_argument("--beam_data", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="results/grpo_v4_9")
    ...
    parser.add_argument("--top_m", type=int, default=8, help="精排的候选数量")
    parser.add_argument("--refine_type", type=str, default="attn", choices=["attn", "mlp"], help="精排类型")
    parser.add_argument("--use_gru", action="store_true", default=True)
    parser.add_argument("--no_gru", dest="use_gru", action="store_false")
    parser.add_argument("--use_step_emb", action="store_true", default=True)
    ...
    train_data, val_data = split_beam_data(beam_data, train_ratio=0.9)
    ...
    print(f"  - refine_type: {args.refine_type}")
    ...
    model = PointerSelectorV4_9_RL(
        ...
        num_layers=args.num_layers, top_m=args.top_m, refine_type=args.refine_type,
        use_gru=args.use_gru, use_step_emb=args.use_step_emb,
        label_smoothing=0.0, dropout=0.5
    )
    
    # 创建训练器
    trainer = GRPOTrainerV4_9(
