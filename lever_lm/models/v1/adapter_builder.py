"""
适配器构建函数：创建适配 v0 训练流程的 v1 模型
"""
from typing import Optional
from omegaconf import DictConfig, OmegaConf

from ..adapter import PointerSelectorAdapter
from .pointer_selector_v1 import PointerSelectorV1, PointerSelectorV1Config, build_model_v1


def build_model_v1_with_adapter(
    config: DictConfig,
    ...
    **kwargs
    ...
    Args:
        config: PointerSelectorV1Config 参数（DictConfig 或已实例化的对象）
        clip_name: CLIP 模型名称
    ...
    Returns:
        PointerSelectorAdapter: 适配器包装的 v1 模型
    """
    # 处理配置参数：可能是 DictConfig 或已实例化的 PointerSelectorV1Config
    if isinstance(config, PointerSelectorV1Config):
        # 已经实例化，直接使用
        pointer_config = config
    else:
        # 是 DictConfig，需要转换
        config_dict = OmegaConf.to_container(config, resolve=True)
        pointer_config = PointerSelectorV1Config(
            d_model=config_dict['d_model'],
            K=config_dict['K'],
            shot_num=config_dict['shot_num'],
            label_smoothing=config_dict.get('label_smoothing', 0.0),
            dropout=config_dict.get('dropout', 0.08),
            temperature=config_dict.get('temperature', 0.5)
        )
    
    # 构建 v1 模型
    pointer_selector = build_model_v1(pointer_config)
