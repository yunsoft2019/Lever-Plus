"""
Pointer Selector V4: V3 + 离线强化学习（GRPO风格后训练）

特点：
- 基础：继承V3的所有功能（灵活架构 + 排序学习）
- 后训练：在V3有监督训练后进行GRPO风格后训练
...
# 导入V3作为基础
from ..v3.pointer_selector_v3 import PointerSelectorV3, PointerSelectorV3Config


class PointerSelectorV4Config:
    """V4模型配置"""
...
class PointerSelectorV4(PointerSelectorV3):
    """
    V4 版本：V3 + 离线强化学习（GRPO）
    ...
    def __init__(
        ...
        print(f"✓ PointerSelectorV4 初始化完成")
        print(f"  - 继承基础: V3 ({base_architecture.upper()})")
    ...
    def forward_with_mode(
        ...
        Args:
            mode: 'supervised' (V3标准), 'rce' (RCE加权), 'grpo' (GRPO-PPO)
        """
        if mode == 'supervised':
            # 标准V3监督学习
...
def build_model_v4(config: PointerSelectorV4Config) -> PointerSelectorV4:
    """构建V4模型的工厂函数"""
    return PointerSelectorV4(**config.to_dict())


def load_v4_from_v3_checkpoint(
    checkpoint_path: str,
    enable_rce: bool = False,
    enable_grpo: bool = False,
    rce_temperature: float = 1.0,
    ...
    """
    从V3 checkpoint加载V4模型（用于后训练）
    
    Args:
        checkpoint_path: V3模型checkpoint路径
        enable_rce: 是否启用RCE
        enable_grpo: 是否启用GRPO
        [其他V4参数...]
    ...
    Returns:
        初始化好的V4模型
    """
    print(f"从 V3 checkpoint 加载 V4 模型...")
    print(f"Checkpoint: {checkpoint_path}")
    
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"找不到V3 checkpoint: {checkpoint_path}")
    ...
    # 检测是V3还是V4 checkpoint
    is_v4_checkpoint = 'enable_rce' in config_dict or 'enable_grpo' in config_dict
    
    if is_v4_checkpoint:
        # 这是V4 checkpoint，允许覆盖V4参数
        print("检测到V4 checkpoint，直接加载...")
        ...
        # 过滤掉不需要的参数（temperature在V4Config中不需要）
        v4_config_dict = {k: v for k, v in config_dict.items() if k != 'temperature'}
        
        # 创建V4配置
        v4_config = PointerSelectorV4Config(**v4_config_dict)
        # 创建V4模型
        model = build_model_v4(v4_config)
        # 加载V4权重
        ...
        print("✓ V4模型已从V4 checkpoint加载")
    else:
        # 这是V3 checkpoint，转换为V4
        print("检测到V3 checkpoint，转换为V4...")
        
        # 创建V4配置（继承V3配置 + V4参数）
        v4_config = PointerSelectorV4Config(
