"""
Pointer Selector V4-2 RL: V4-2 + 离线强化学习（RCE预热 + GRPO后训练）

特点：
- 继承V4-2的GRU Pointer Decoder架构
- 复用V3的RCE/GRPO方法
- 支持组内相对优势计算
- 支持KL散度计算和自适应调整
...
from ..v2.pointer_selector_v4_2 import PointerSelectorV4_2


class PointerSelectorV4_2_RL(PointerSelectorV4_2):
    """
    V4-2 RL 版本：V4-2 + 离线强化学习（RCE预热 + GRPO后训练）
    
    继承V4-2的GRU Pointer Decoder架构，新增强化学习相关方法：
    ...
    def __init__(
        ...
        print(f"✓ PointerSelectorV4_2_RL 初始化完成（V4-2 + 强化学习）")
...
class PointerSelectorV4_2_RLConfig:
    """V4-2 RL 模型配置类"""
...
def build_model_v4_2_rl(config: Optional[PointerSelectorV4_2_RLConfig] = None) -> PointerSelectorV4_2_RL:
    """构建 V4-2 RL 模型的工厂函数"""
    if config is None:
        config = PointerSelectorV4_2_RLConfig()
...
if __name__ == "__main__":
    """测试代码"""
    print("="*70)
    print("测试 PointerSelectorV4_2_RL 模型")
    print("="*70)
    
    model = build_model_v4_2_rl()
