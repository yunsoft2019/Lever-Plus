"""
Pointer Selector V4-3 RL: V4-3 + 离线强化学习（RCE预热 + GRPO后训练）

特点：
- 继承V4-3的GRU Pointer Decoder + MMR多样性残差架构
- 复用V3的RCE/GRPO方法
- 支持组内相对优势计算
- 支持KL散度计算和自适应调整
...
from ..v2.pointer_selector_v4_3 import PointerSelectorV4_3


class PointerSelectorV4_3_RL(PointerSelectorV4_3):
    """
    V4-3 RL 版本：V4-3 + 离线强化学习（RCE预热 + GRPO后训练）
    
    继承V4-3的GRU Pointer Decoder + MMR架构，新增强化学习相关方法：
    ...
    def __init__(
        ...
        print(f"✓ PointerSelectorV4_3_RL 初始化完成（V4-3 + 强化学习）")
...
class PointerSelectorV4_3_RLConfig:
    """V4-3 RL 模型配置类"""
...
def build_model_v4_3_rl(config: Optional[PointerSelectorV4_3_RLConfig] = None) -> PointerSelectorV4_3_RL:
    """构建 V4-3 RL 模型的工厂函数"""
    if config is None:
        config = PointerSelectorV4_3_RLConfig()
...
if __name__ == "__main__":
    """测试代码"""
    print("="*70)
    print("测试 PointerSelectorV4_3_RL 模型")
    print("="*70)
    
    model = build_model_v4_3_rl()
