"""
Pointer Selector V4-5 RL: V4-5 + 离线强化学习（RCE预热 + GRPO后训练）

特点：
- 继承V4-5的GRU Pointer Decoder + Additive/Bilinear Attention架构
- 复用V3的RCE/GRPO方法
- 支持组内相对优势计算
- 支持KL散度计算和自适应调整
...
from ..v2.pointer_selector_v4_5 import PointerSelectorV4_5


class PointerSelectorV4_5_RL(PointerSelectorV4_5):
    """
    V4-5 RL 版本：V4-5 + 离线强化学习（RCE预热 + GRPO后训练）
    
    继承V4-5的GRU Pointer Decoder + Additive/Bilinear Attention架构，新增强化学习相关方法：
    ...
    def __init__(
        ...
        print(f"✓ PointerSelectorV4_5_RL 初始化完成（V4-5 + 强化学习）")
...
class PointerSelectorV4_5_RLConfig:
    """V4-5 RL 模型配置类"""
...
def build_model_v4_5_rl(config: Optional[PointerSelectorV4_5_RLConfig] = None) -> PointerSelectorV4_5_RL:
    """构建 V4-5 RL 模型的工厂函数"""
    if config is None:
        config = PointerSelectorV4_5_RLConfig()
...
if __name__ == "__main__":
    """测试代码"""
    print("="*70)
    print("测试 PointerSelectorV4_5_RL 模型")
    print("="*70)
    ...
    for attn_type in ['additive', 'bilinear']:
        print(f"\n{'='*70}")
        print(f"测试 attention_type = '{attn_type}'")
        print("="*70)
        
        config = PointerSelectorV4_5_RLConfig(attention_type=attn_type)
        model = build_model_v4_5_rl(config)
