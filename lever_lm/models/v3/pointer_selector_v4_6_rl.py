"""
Pointer Selector V4-6 RL: V4-6 + 离线强化学习（RCE预热 + GRPO后训练）

特点：
- 继承V4-6的GRU Pointer Decoder + Coverage / Topic 原型覆盖架构
- 复用V3的RCE/GRPO方法
- 支持组内相对优势计算
- 支持KL散度计算和自适应调整
...
from ..v2.pointer_selector_v4_6 import PointerSelectorV4_6


class PointerSelectorV4_6_RL(PointerSelectorV4_6):
    """
    V4-6 RL 版本：V4-6 + 离线强化学习（RCE预热 + GRPO后训练）
    
    继承V4-6的GRU Pointer Decoder + Coverage / Topic 原型覆盖架构，新增强化学习相关方法：
    ...
    def __init__(
        ...
        Args:
            ...
            cover_lambda_init: 覆盖增益权重初始值 (默认 0.0)
        ...
        super().__init__(
            ...
            cover_lambda_init=cover_lambda_init
        )
        ...
        print(f"✓ PointerSelectorV4_6_RL 初始化完成（V4-6 + 强化学习）")
...
class PointerSelectorV4_6_RLConfig:
    """V4-6 RL 模型配置类"""
...
def build_model_v4_6_rl(config: Optional[PointerSelectorV4_6_RLConfig] = None) -> PointerSelectorV4_6_RL:
    """构建 V4-6 RL 模型的工厂函数"""
    if config is None:
        config = PointerSelectorV4_6_RLConfig()
...
if __name__ == "__main__":
    """测试代码"""
    print("="*70)
    print("测试 PointerSelectorV4_6_RL 模型")
    print("="*70)
    ...
    for num_topics in [8, 16, 32]:
        ...
        model = build_model_v4_6_rl(config)
