"""
Pointer Selector V4-4 RL: V4-4 + 离线强化学习（RCE预热 + GRPO后训练）

特点：
- 继承V4-4的Candidate Set Encoder + GRU Pointer Decoder + MMR架构
- 复用V3的RCE/GRPO方法
- 支持组内相对优势计算
- 支持KL散度计算和自适应调整
...
from ..v2.pointer_selector_v4_4 import PointerSelectorV4_4


class PointerSelectorV4_4_RL(PointerSelectorV4_4):
    """
    V4-4 RL 版本：V4-4 + 离线强化学习（RCE预热 + GRPO后训练）
    
    继承V4-4的Candidate Set Encoder + GRU + MMR架构，新增强化学习相关方法
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            use_gru=use_gru,
            mmr_reduction=mmr_reduction,
            cand_encoder_layers=cand_encoder_layers,
            cand_encoder_heads=cand_encoder_heads
        )
        ...
        print(f"✓ PointerSelectorV4_4_RL 初始化完成（V4-4 + 强化学习）")
...
class PointerSelectorV4_4_RLConfig:
    """V4-4 RL 模型配置类"""
...
def build_model_v4_4_rl(config: Optional[PointerSelectorV4_4_RLConfig] = None) -> PointerSelectorV4_4_RL:
    """构建 V4-4 RL 模型的工厂函数"""
    if config is None:
        config = PointerSelectorV4_4_RLConfig()
    
    return PointerSelectorV4_4_RL(**config.to_dict())


if __name__ == "__main__":
    """测试代码"""
    print("="*70)
    print("测试 PointerSelectorV4_4_RL 模型")
    print("="*70)
    
    model = build_model_v4_4_rl()
