# 改进 3.3 实现总结

> 实现时间：2025-12-10  
> 基于：`LeverPlus_v3_RL_next_steps_2025_12_10.md` 第 3.3 节

---

## ✅ 3.3.1 添加 `vqa_eval_mode` 字段

### 修改位置

**文件：** `lever_lm/models/v3/generate_rl_data.py`

**修改内容：**

1. **在添加 correctness 信息时添加 `vqa_eval_mode` 字段**（第616行）：
   ```python
   c["vqa_pred_answer"] = pred_answer
   c["vqa_correct"] = correct
   c["vqa_acc_score"] = acc_score
   c["vqa_eval_mode"] = "file" if used_file_metric else "fallback"  # 新增
   ```

2. **在异常处理时也添加 `vqa_eval_mode` 字段**（第625行）：
   ```python
   c["vqa_pred_answer"] = ""
   c["vqa_correct"] = 0
   c["vqa_acc_score"] = 0.0
   c["vqa_eval_mode"] = "fallback"  # 异常情况标记为 fallback
   ```

**说明：**
- `vqa_eval_mode` 字段标记 reward 质量来源：
  - `"file"`：使用官方 VQA metric 计算（更可靠）
  - `"fallback"`：使用字符串匹配计算（备用方案）
- 统计信息已在第636-642行打印，无需额外修改

---

## ✅ 3.3.2 在 Dataset 中支持跳过 fallback 样本

### 修改位置

**文件：** `lever_lm/models/v3/dataset_v3.py`

**修改内容：**

1. **在 `__init__` 方法中添加 `skip_fallback_reward` 参数**（第347行）：
   ```python
   skip_fallback_reward: bool = False  # 3.3.2: 是否跳过使用 fallback 方式计算的 RL 样本
   ```

2. **在文档字符串中添加参数说明**（第371行）：
   ```python
   skip_fallback_reward: 是否跳过使用 fallback 方式计算的 RL 样本（默认 False，推荐在 OKVQA 上打开）
   ```

3. **保存参数**（第385行）：
   ```python
   self.skip_fallback_reward = skip_fallback_reward  # 3.3.2: 保存参数
   ```

4. **在遍历 `pointer_candidates` 时添加过滤逻辑**（第415-417行）：
   ```python
   for c in pointer_candidates:
       # 3.3.2: 如果要求跳过 fallback 样本
       if self.skip_fallback_reward and c.get("vqa_eval_mode") == "fallback":
           continue
       
       pointer = c["pointer"]
       ...
   ```

5. **添加检查：如果过滤后没有任何候选，跳过这个 query**（第468-470行）：
   ```python
   # 3.3.2: 如果过滤后没有任何候选，跳过这个 query
   if len(beam_labels) == 0:
       continue
   ```

6. **在打印信息中显示 `skip_fallback_reward` 状态**（第483-486行）：
   ```python
   if self.skip_fallback_reward:
       print(f"  - skip_fallback_reward: True（已过滤 fallback 样本，只使用官方 VQA metric）")
   else:
       print(f"  - skip_fallback_reward: False（使用所有样本，包括 fallback）")
   ```

---

## ✅ 3.3.3 在训练脚本中暴露开关

### 修改位置

**文件：** `lever_lm/workflows/grpo_post_train.py`

**修改内容：**

1. **在 argparse 中添加 `--skip_fallback_reward` 参数**（第532-533行）：
   ```python
   # 3.3.3: 跳过 fallback 样本开关
   parser.add_argument("--skip_fallback_reward", action="store_true",
                       help="是否跳过使用 fallback 方式计算的 RL 样本（推荐在 OKVQA 上打开）")
   ```

2. **在构造 dataset 时传入参数**（第642行和第659行）：
   ```python
   train_dataset = RLBeamDatasetWithEmbedding(
       ...,
       skip_fallback_reward=args.skip_fallback_reward  # 3.3.3: 传入 skip_fallback_reward 参数
   )
   
   val_dataset = RLBeamDatasetWithEmbedding(
       ...,
       skip_fallback_reward=args.skip_fallback_reward  # 3.3.3: 传入 skip_fallback_reward 参数
   )
   ```

3. **在 main() 函数中打印状态**（第825-828行）：
   ```python
   # 3.3.3: 打印 skip_fallback_reward 状态
   if args.skip_fallback_reward:
       print("✓ 已启用 skip_fallback_reward：训练时将跳过 fallback 样本，只使用官方 VQA metric")
   else:
       print("✓ skip_fallback_reward 未启用：训练时将使用所有样本（包括 fallback）")
   ```

---

## ✅ 更新 `train_v3.sh` 脚本

### 修改位置

**文件：** `scripts/train_v3.sh`

**修改内容：**

1. **在注释中添加 `SKIP_FALLBACK_REWARD` 环境变量说明**（第34行）：
   ```bash
   #   SKIP_FALLBACK_REWARD: 跳过使用 fallback 方式计算的 RL 样本（默认: false，推荐在 OKVQA 上打开）
   ```

2. **读取环境变量**（第158行）：
   ```bash
   skip_fallback_reward=${SKIP_FALLBACK_REWARD:-false}
   ```

3. **在配置打印中显示状态**（第194-196行）：
   ```bash
   if [ "${skip_fallback_reward}" == "true" ]; then
       echo "  Skip Fallback: 跳过 fallback 样本，只使用官方 VQA metric"
   fi
   ```

4. **在训练命令中添加参数**（第286-289行）：
   ```bash
   # 3.3.3: 如果指定跳过 fallback 样本
   if [ "${skip_fallback_reward}" == "true" ]; then
       train_cmd="${train_cmd} --skip_fallback_reward"
   fi
   ```

---

## 📝 使用方法

### 1. 直接使用 Python 脚本

```bash
# 启用 skip_fallback_reward
python -m lever_lm.workflows.grpo_post_train \
    --beam_data ./results/okvqa/generated_data/rl_data_RandSampler_Qwen2_5-VL-3B-Instruct.json \
    --img_emb ./results/okvqa/cache/query_embeddings.pt \
    --output_dir ./results/okvqa/model_cpk/v3_test \
    --skip_fallback_reward \
    --device cuda:0
```

### 2. 使用 `train_v3.sh` 脚本

```bash
# 启用 skip_fallback_reward
export SKIP_FALLBACK_REWARD=true
bash scripts/train_v3.sh vqa okvqa_local 0 query_img_text_icd_img_text rand_sampler qwen2.5_vl_3B
```

---

## 🎯 实现效果

1. **RL 数据质量标记**：
   - 每个 pointer candidate 都有 `vqa_eval_mode` 字段
   - 可以清楚知道 reward 的来源（官方 metric 还是 fallback）

2. **训练数据过滤**：
   - 可以选择只使用官方 VQA metric 计算的 reward
   - 提高训练数据的质量和可靠性

3. **向后兼容**：
   - 默认不启用 `skip_fallback_reward`（`False`）
   - 不影响现有训练流程
   - 旧数据如果没有 `vqa_eval_mode` 字段，会被当作非 fallback 处理（`c.get("vqa_eval_mode") == "fallback"` 返回 `False`）

---

## ✅ 验证

所有修改已完成，且通过 linter 检查：
- ✅ `lever_lm/models/v3/generate_rl_data.py`
- ✅ `lever_lm/models/v3/dataset_v3.py`
- ✅ `lever_lm/workflows/grpo_post_train.py`
- ✅ `scripts/train_v3.sh`

---

**生成时间：** 2025-12-10  
**状态：** ✅ 完全实现





